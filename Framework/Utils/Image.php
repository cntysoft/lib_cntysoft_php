...
class Image
{
   ...
   public function __construct(array $imagePath, $options = array())
   {
      ...
      if (@!($imageSizeInfos = getimagesize($imagePath['imageFromPath']))) {
         ...
         Kernel\throw_exception(new Exception(
                 $errorType->msg('E_IMAGE_TO_DEAL_NOT_EXIT'), $errorType->code('E_IMAGE_TO_DEAL_NOT_EXIT')), $errorType);
      }
      ...
      switch ($mimeContentType) {
         ...
         default:
            $errorType = ErrorType::getInstance();
            Kernel\throw_exception(new Exception(
                    $errorType->msg('E_IMAGE_TYPE_NOT_EXIT'), $errorType->code('E_IMAGE_TYPE_NOT_EXIT')), $errorType);
            break;
      }
   ...
   public function cropMaximumImage($path, $filename)
   {
      ...
      $this->iwsImage->cropInPixel($size, $size, 0, 0, 'MM');
      $this->iwsImage->save($path, $filename, false, null, 100);
   }
   ...
   protected function checkRequireFields(array &$params = array(), array $requires = array())
   {
      ...
      if (!empty($leak)) {
         Kernel\throw_exception(new Exception(
                 Kernel\StdErrorType::msg('E_API_INVOKE_LEAK_ARGS', implode(', ', $leak)), Kernel\StdErrorType::code('E_API_INVOKE_LEAK_ARGS')), \Cntysoft\STD_EXCEPTION_CONTEXT);
      }
   }
