...
class Manager implements EventsAwareInterface
{
     ...
     */
    public function addTempFileRef($churchId, array $refInfo)
    {
     ...
     * @return string
     */
    protected static function getRefInfoModelCls($tableId)
    {
    ...
    public static function getAttachmentFilename($filename)
    {
        if (PHP_OS == \Cntysoft\WINDOWS) {
            $dirname = StdDir::getUploadDir().DS.date('Y'.DS.DS.'m'.DS.DS.'d');
        } else {
            $dirname = StdDir::getUploadDir().DS.date('Y'.DS.'m'.DS.'d');
        }
