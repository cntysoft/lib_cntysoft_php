...
//        return $responseText;
   ...
   {
      switch (strtoupper(trim($params['sign_type']))) {
         case self::PARAM_SIGN_TYPE_MD5:
            $ret = $this->generateMD5Sign($params);
            break;
         case self::PARAM_SIGN_TYPE_RSA:
            $ret = $this->generateRSASign($params);
            break;
      }
   ...
   public function verifyNotifySign($params, $notifySign, $signType = 'MD5')
   {
      switch (strtoupper(trim($signType))) {
         case self::PARAM_SIGN_TYPE_MD5:
            $ret = $this->verifyMD5Sign($params, $notifySign);
            break;
         case self::PARAM_SIGN_TYPE_RSA:
            $ret = $this->verifyRSASign($params, $notifySign);
            break;
      }
    ...
    * @return boolean
    */
   protected function verifyMD5Sign($params, $notifySign)
   {
    ...
    * @return string
    */
   protected function getParamsSign($params)
   {
      //首先进行排序，组装
