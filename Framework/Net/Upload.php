...
use Christ\Framework\Core\FileRef\Manager as FileRefManager;
...
class Upload
{
    ...
    public function saveUploadFile($uploadFile)
    {
        ...
        $ret = array();
        if ($this->options->getEnableFileRef()) {
            $refManager = new FileRefManager();
            foreach ($savedFiles as $file) {
                $item = array();
                $stat = stat($file);
                try {
                    $rid = $refManager->addTempFileRef(null, array(
                       'filesize'   => $stat['size'],
                       'filename'   => Kernel\convert_2_utf8(Filesystem::basename($file)),
                       'attachment' => Kernel\convert_2_utf8(str_replace(CNTY_ROOT_DIR, '', $file))
                    ));
                    $item['rid'] = $rid;
                    $item['filename'] = Kernel\convert_2_utf8(str_replace(DS, '/', str_replace(CNTY_ROOT_DIR, '', $file)));
                    array_push($ret, $item);
                } catch (\Exception $e) {
    ...
    {
        //目标上传文件夹
        //$targetDir = dirname($targetFile);  这行代码在WINDOWS系统下对中文路径操作有问题
