...
class TagRefl
{
   ...
   public static function getTagList(array $tagTypes = array())
   {
      ...
      $path = $tagResolver->getTagBaseDir();
      $cacheKey = md5($path);
      if (!isset(self::$tagListCache[$cacheKey])) {
         $ret = array();
         ...
         self::$tagListCache[$cacheKey] = $ret;
      }
      return self::$tagListCache[$cacheKey];
   }
   ...
   /**
    * 根据 $newClassify 修改或新加一个分类
    *
    * @param type $tagType
    * @param type $classify 旧的分类名称
    * @param type $newClassify 新的分类名称
    */
   public static function classifyChange($tagType, $classify, $newClassify)
   {
      $tagResolver = View::getTagResolver();
      ...
      if('' === $classify){
         ...
         $target = $baseDir.DS.$tagType.DS.$newClassify;
         $source = $baseDir.DS.$tagType.DS.$classify;
...
//            self::createLabelSubDirs($dir);
   ...
   public static function getTagClassifies($type = self::T_LABLE)
   {
      ...
      $tagResolver = View::getTagResolver();
      $baseDir = $tagResolver->getTagBaseDir();
      $path = Kernel\real_path($baseDir.DS.$type);
      $ret = array();
